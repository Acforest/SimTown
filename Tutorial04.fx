...
float4 PS_Lambertian_Shading(PS_INPUT input) : SV_Target
{
    ...
    for (int i = 0; i < 3; i++)
    {
        finalColor += saturate(input.Color * dot(normalize((float3) vLightDir[i]), input.Norm) * vLightColor[i]);
    }
...
{
    float4 finalColor;
    float3 bisector = normalize((float3) (normalize(vCamera - input.Pos) + vLightDir[0])); // 相机视方向与光线方向的二分线
    int x = 5;  // 高光指数
    float4 sc = { 1.0f, 1.0f, 1.0f, 1.0f }; // 高光颜色
    finalColor = input.Color * vLightColor[0] * max(dot(normalize((float3) vLightDir[0]), input.Norm), 0) + sc * vLightColor[0] * pow(max(dot(bisector, input.Norm), 0), x);
    finalColor.a = 1.0f;
    return finalColor;
}
...
float4 PS_Texture_Mapping(PS_INPUT input) : SV_Target
{
    ...
    float4 finalColor = saturate(tex_color * dot(normalize((float3) vLightDir[0]), input.Norm));
    finalColor.a = 1.0f;
    return finalColor;
}