...
public:
    FreeLookCamera(XMVECTOR& mPosition, XMVECTOR& mTarget, XMVECTOR& mUpAxis, XMVECTOR& mRightAxis)
    {
    ...
    {
        float normalize_Length = mTarget.x * mTarget.x + mTarget.y * mTarget.y + mTarget.z * mTarget.z;
        float normalize_X = mTarget.x / normalize_Length;
        float normalize_Y = mTarget.y / normalize_Length;
        float normalize_Z = mTarget.z / normalize_Length;
        mPosition.x += normalize_X * d;
        mPosition.y += normalize_Y * d;
        mPosition.z += normalize_Z * d;
    }
    // 抬头低头（绕局部坐标系的X轴旋转）
